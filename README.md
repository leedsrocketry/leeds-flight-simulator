...
- [Debugging and Iteration](#debugging-and-iteration)
- [Contact](#contact)
- [Licence](#licence)
...
When setting up a new vehicle or site configuration, **start with all Monte Carlo uncertainties set to zero** and introduce them one at a time. This applies to both the LFS simulation config and the wind profiles generated by windgen.

### Why

With uncertainties at zero every sample follows the same trajectory, so you can focus on whether the baseline physics, acceptance criteria, and geofencing are correct before the stochastic spread makes problems harder to diagnose. It also makes iteration dramatically faster — see below.
...
**Some uncertainties can increase runtime by an order of magnitude.** The adaptive step-size integrator chooses its step size based on the fastest-changing quantity in the state vector. Uncertainties that excite high-frequency dynamics force the integrator to take far more steps:

A full 4000-sample run with all uncertainties active can take 15+ minutes. The same run with `fin_cant_sigma: 0.0` and deterministic wind may complete in under a minute. This makes it far more practical to iterate on configuration, acceptance thresholds, and site geometry before committing to a full stochastic analysis.

### Suggested workflow

1. Set all uncertainties to zero and `samples` to a small number (e.g. 10).
2. Run and verify the baseline trajectory behaves as expected (apogee, landing, containment).
3. Introduce `impulse_factor_sigma` and `azimuth_sigma`/`inclination_sigma`. These are cheap and show the effect of motor and rail alignment uncertainty.
4. Introduce wind perturbations (`--perturbation-scale 1.0` in windgen) and increase `samples` to match the ensemble size. Check that the landing dispersion is reasonable.
5. Finally, introduce `fin_cant_sigma`. This is the most expensive uncertainty and should be added last. If iteration speed matters, consider using a reduced sample count until the configuration is finalised, then run the full analysis once.
